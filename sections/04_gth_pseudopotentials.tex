...
From a chemical point of view, this effort is often unnecessary. Core electrons are strongly bound and usually change little during bond formation, whereas valence electrons determine most chemical properties. A pseudopotential therefore replaces the nucleus and its chemically inactive core electrons by an effective ionic core, as shown schematically in \cref{fig:gth_replacement}.

The effective ionic core should be understood as the net object seen by a valence electron. In the all-electron description, this valence electron is attracted by the nucleus and repelled by the core electrons. The core electrons therefore screen part of the nuclear charge, so that far from the atom the combined interaction corresponds to an attraction by the reduced charge $Z_{\text{ion}}=Z-N_{\text{core}}$. Close to the core, however, the pseudopotential also represents quantum-mechanical effects associated with the removed core states, such as orthogonality to core orbitals, angular-momentum-dependent scattering and the exchange-correlation treatment used in the atomic reference calculation. It is therefore better viewed as an effective valence-electron--ionic-core interaction than as a simple sum of nuclear attraction and classical electron--electron repulsion.

\begin{Figure}
    \centering
    \input{img/fig7_gth_replacement}
    \captionof{figure}{Conceptual replacement made by a pseudopotential. In the all-electron description, the valence electrons interact with the bare nucleus while the core electrons are treated explicitly. In the pseudopotential description, the nucleus and frozen core electrons are replaced by an effective ionic core that acts on the explicitly treated valence electrons.}
    \label{fig:gth_replacement}
\end{Figure}

Only the valence electrons are treated explicitly after this replacement. Away from the core, a pseudo-valence orbital closely reproduces the corresponding all-electron valence orbital. In the core region, it is replaced by a smooth, nodeless function, as illustrated in \cref{fig:gth_pseudo_orbital}. The interaction responsible for producing this pseudo-orbital is the \textit{pseudopotential}.

\begin{Figure}
    \centering
    \includegraphics[width=\linewidth]{img/fig8_pseudo_orbital.pdf}
    \captionof{figure}{Signed radial distribution functions $r^{2}\psi_{ns}(r)$ from self-consistent, spherical atomic LDA calculations for Si. The inset magnifies the all-electron core region on the same linear amplitude scale. The occupied all-electron 1s, 2s and 3s functions have zero, one and two radial nodes, respectively. The latter nodes enforce orthogonality of 3s to the two lower s states. A four-valence-electron calculation with the GTH-PADE-q4 potential instead gives a smooth, nodeless pseudo-3s function that closely follows the all-electron 3s function away from the core. Every curve uses the same scale factor.}
    \label{fig:gth_pseudo_orbital}
\end{Figure}
...
In the pseudopotential calculation, the bare external electron--nuclear potential $\nu_{\text{ext}}$ is replaced by the GTH electron--ion operator. This operator is divided into a local and a non-local part,
...
Thus, $\hat{V}_{\text{GTH}}$ is the pseudopotential counterpart of $\nu_{\text{ext}}$. The replacement is not merely a change of notation: the all-electron Coulomb attraction to a point nucleus is exchanged for an effective interaction between the explicitly treated valence electrons and the ionic core.

In this context, \textit{local} does not merely mean localized near a nucleus. It means that the operator is an ordinary function of position: at a given point in space it multiplies the orbital value at that same point. The local part therefore acts in the same mathematical way as the bare external potential, although with a different functional form. It represents the long-range attraction between a valence electron and an ionic core, together with a smooth correction close to that core.

The non-local part is different because its action at one point is not determined solely by the orbital value at that same point. Instead, it distinguishes between different angular characters. Close to a carbon atom, for example, a portion of an orbital resembling an $s$ orbital need not experience exactly the same effective interaction as a portion resembling a $p$ orbital. Because an arbitrary molecular or crystalline orbital does not possess one definite atomic angular momentum, the operator first measures how strongly the orbital overlaps atom-centred functions of a particular angular character. It subsequently weights these overlaps and adds the corresponding functions back to the orbital. These measuring functions are termed \textit{projectors}.
